...
- **핵심 패키지** (`backend/requirements.txt` 에 고정):
  ...
  - `python-dotenv`
  - `openai` (러버덕/파싱/인사이트 LLM — GitHub Models 또는 Azure OpenAI)
- **개발 도구**:
  - `pytest` (테스트)
  - `ruff` (린트/포매팅)
- **가상환경**:
...
| `COPILOT_SDK_TOKEN` | GitHub Copilot SDK 토큰(GitHub Models, OpenAI 호환). PAT `models:read` 권한 | (비밀, 커밋 금지) |
| `COPILOT_SDK_BASE_URL` | (선택) GitHub Models 엔드포인트 | `https://models.github.ai/inference` |
| `COPILOT_SDK_MODEL` | (선택) 사용할 모델 id | `openai/gpt-4o-mini` |
| `AZURE_OPENAI_API_KEY` | (대안) Azure OpenAI 키 — 설정 시 Copilot보다 우선 | (비밀, 커밋 금지) |
| `AZURE_OPENAI_ENDPOINT` | (대안) Azure OpenAI 엔드포인트 | `https://<resource>.openai.azure.com` |
| `AZURE_OPENAI_DEPLOYMENT` | (대안) 배포(모델) 이름 | `gpt-4o-mini` |
